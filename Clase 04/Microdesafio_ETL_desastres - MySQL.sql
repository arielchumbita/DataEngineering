...
BEGIN
  DELETE FROM desastres_bde.desastres_final; /*Vacio tabla*/
  INSERT INTO desastres_bde.desastres_final
    SELECT 
      ...
      AVG(R_30_a_45 + R_45_a_60) AS M_Adultos_AVG,
      AVG(R_M_a_60) AS M_Ancianos_AVG
    FROM desastres.clima AS c
    JOIN desastres.desastres AS d ON c.año = d.año
    JOIN desastres.muertes AS m ON c.año = m.año
    group by Cuatrenio;
END $$
DELIMITER ;

--  OPCIÓN QUERY CON SUBCONSULTA
DELIMITER $$
CREATE PROCEDURE pETL_Desastres()
BEGIN
  DELETE FROM desastres_bde.desastres_final; /*Vacio tabla*/
  INSERT INTO desastres_bde.desastres_final
    SELECT 
      ...
      avg(x.oxigeno) AS Oxi_AVG,
      sum(x.tsunamis) AS T_Tsunamis,
      sum(x.Olas_Calor) AS T_OlasCalor,
      sum(x.Terremotos) AS T_Terremotos,
      sum(x.Erupciones) AS T_Erupciones,
      sum(x.Incendios) AS T_Incendios,
      AVG(x.M_Jovenes) AS M_Jovenes_AVG,
      AVG(x.M_Adultos) AS M_Adultos_AVG,
      AVG(x.M_Ancianos) AS M_Ancianos_AVG
    FROM
		(select
			CASE
			...
			c.oxigeno oxigeno,
			d.Tsunamis tsunamis,
			d.Olas_calor Olas_Calor,
			d.Terremotos Terremotos,
			d.Erupciones Erupciones,
			d.Incendios Incendios,
			m.R_Menor15 + m.R_15_a_30 AS M_Jovenes, /* se define alias aqui */
...
select*from desastres.clima;
select*from desastres.desastres;
select*from desastres.muertes;
...
-- Desactivar safe updates mode solo para MySQL
